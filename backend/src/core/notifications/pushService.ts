...
export async function notifyUser(userId: string, payload: PushPayload): Promise<void> {
  try {
    const user = await UserModel.findById(userId).select('pushTokens notificationSettings name').lean();
    if (!user) return;
    if (user.notificationSettings && user.notificationSettings.inApp === false) return;

    const tokens = Array.isArray((user as any).pushTokens)
    ...
    const bad = tickets
      .map((ticket: any, index: number) => ({ ticket, token: expoTokens[index] }))
      .filter(({ ticket }) => ticket?.status === 'error');

    if (bad.length) {
      const drop = bad
        .filter(({ ticket }) => /DeviceNotRegistered|InvalidCredentials/i.test(String(ticket?.details?.error || ticket?.message || '')))
        .map(({ token }) => token);
      if (drop.length) {
